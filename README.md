...
Grab `LoLeanding.exe` from the [latest release](https://github.com/ElOsKr/lol-tracker/releases/latest) and run it. It's a portable Windows build with no installer, so it can live anywhere.
...
Match data lives in `%APPDATA%\loleanding\data`, with automatic backups alongside it. An install from before the rename keeps working: on first launch the app moves its games and backups over from `%APPDATA%\mayhem-tracker`. The app checks GitHub for new releases and offers to update itself when one lands.
